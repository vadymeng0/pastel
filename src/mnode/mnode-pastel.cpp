 ...
 */
void trade_copy_validation(const std::string& nftTnxId, const v_uint8& signature) {
//  if (!masterNodeCtrl.masternodeSync.IsSynced()) {
...
NFTSellTickets_t CNFTSellTicket::FindAllTicketByNFTTnxID(const std::string& NFTTnxId)
{
...
CNFTTradeTicket CNFTTradeTicket::Create(std::string _sellTnxId, std::string _buyTnxId, std::string _pastelID, const SecureString& strKeyPass)
{
    ...
    auto NFTRegTicket_TxId_Serial = CNFTTradeTicket::GetNFTRegTxIDAndSerialIfResoldNft(sellTicket->NFTTnxId);
...
NFTTradeTickets_t CNFTTradeTicket::FindAllTicketByNFTTnxID(const std::string& NFTTnxID)
{
    return masterNodeCtrl.masternodeTickets.FindTicketsByMVKey<CNFTTradeTicket>(NFTTnxID);
}

NFTTradeTickets_t CNFTTradeTicket::FindAllTicketByRegTnxID(const std::string& nftRegTnxId)
{
...
NFTRoyaltyTickets_t CNFTRoyaltyTicket::FindAllTicketByNFTTnxId(const std::string& NFTTnxId)
{
